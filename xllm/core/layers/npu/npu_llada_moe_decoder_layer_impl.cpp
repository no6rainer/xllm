...
#include "npu_llada_moe_decoder_layer_impl.h"
...
NpuLLaDAMoeDecoderLayerImpl::NpuLLaDAMoeDecoderLayerImpl(
...
void NpuLLaDAMoeDecoderLayerImpl::initialize_tensors(
...
void NpuLLaDAMoeDecoderLayerImpl::resize_experts_weights(
...
void NpuLLaDAMoeDecoderLayerImpl::initialize_weight_tensors(
    const torch::TensorOptions& options) {
...
void NpuLLaDAMoeDecoderLayerImpl::load_state_dict(const StateDict& state_dict) {
...
int NpuLLaDAMoeDecoderLayerImpl::get_mapped_index(
...
void NpuLLaDAMoeDecoderLayerImpl::process_expert_weights(
...
void NpuLLaDAMoeDecoderLayerImpl::process_mlp_common_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
...
void NpuLLaDAMoeDecoderLayerImpl::process_general_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
...
torch::Tensor NpuLLaDAMoeDecoderLayerImpl::get_sharded_tensor(
    const StateDict& state_dict,
    const std::string& name,
    int dim) {
...
std::string NpuLLaDAMoeDecoderLayerImpl::extract_endswith(
...
int NpuLLaDAMoeDecoderLayerImpl::extract_expert_index(const std::string& name) {
...
void NpuLLaDAMoeDecoderLayerImpl::verify_loaded_weights(
...
void NpuLLaDAMoeDecoderLayerImpl::merge_loaded_weights() {
...
torch::Tensor NpuLLaDAMoeDecoderLayerImpl::convert_fp16_to_int64(
...
void NpuLLaDAMoeDecoderLayerImpl::convert_descaled_weights_to_float() {
...
void NpuLLaDAMoeDecoderLayerImpl::merge_experts_weights() {
...
torch::Tensor NpuLLaDAMoeDecoderLayerImpl::merge_experts_weights(
...
int64_t NpuLLaDAMoeDecoderLayerImpl::init_layer() {
  name_ = "llada_moe_decoder_layer " + std::to_string(layer_id_);
  model_name_ = "LLaDA_Moe";
...
torch::Tensor NpuLLaDAMoeDecoderLayerImpl::forward(
    torch::Tensor& x,
...
void NpuLLaDAMoeDecoderLayerImpl::build_node_variant_pack(
    atb_speed::Model::Node& node,
    torch::Tensor& x,
    torch::Tensor& cos_pos,
    torch::Tensor& sin_pos,
